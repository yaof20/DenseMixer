...
def _apply_router_policy(model, policy: str = "frozen") -> None:
    """
    ...
    """
    if policy not in {"unfrozen"}:  # default → no-op
        return

    unfrozen_cnt = 0
    # unwrap PEFT → the transformer blocks live under .model.layers
    layers = getattr(getattr(model, "model", model), "model", model).layers
            ...
            if policy == "unfrozen":
                for p in gate.parameters():
                    if not p.requires_grad:
                        ...
                        unfrozen_cnt += p.numel()

    if policy == "unfrozen":
        logger.info_rank0(f"[LoRA] Unfroze {unfrozen_cnt:,} router-gate parameters.")
...
def _setup_lora_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        for adapter in adapter_to_merge:
            if "unfrozen" in adapter:
                
                lora_model = PeftModel.from_pretrained(base, adapter, **init_kwargs)
                # for name in base.state_dict().keys():
                #     if "mlp.gate" in name:
                #         print(name)
              

                gate_state = torch.load(f"{adapter}/gate.pth", map_location="cpu")
                missing, _ = lora_model.load_state_dict(gate_state, strict=False)
                print("missing:", len(missing))

                before = copy.deepcopy(
                    lora_model.base_model.model.model.layers[0].mlp.gate.base_layer.weight
                )

                model = lora_model.merge_and_unload(safe_merge=True)
                # for name in model.state_dict().keys():
                #     if "mlp.gate" in name:
                #         print(name)

                after = model.model.layers[0].mlp.gate.weight

                print("ΔW mean abs =", (after - before).abs().mean())
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        if len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] == "all":
            ...
            if finetuning_args.lora_router != "unfrozen":
                new_target_modules = []
        ...
        if model_args.use_unsloth:
            ...
            _apply_router_policy(model, getattr(finetuning_args, "lora_router", "frozen"))
