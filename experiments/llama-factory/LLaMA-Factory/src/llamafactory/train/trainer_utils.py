...
def save_gate_parameters_only(model, out_dir: str = "gate_ckpt"):
    ...
    for name, param in model.named_parameters():
        if "mlp.gate" not in name or param.numel() == 0:
            continue

        if zero is not None and hasattr(param, "ds_id"):
            with zero.GatheredParameters([param], modifier_rank=0):
                if rank == 0:
                    gate_state[name] = param.detach().cpu().clone()
    ...
    if rank == 0:
        if not gate_state:
            print("[rank-0] ⚠️  No gate tensors found!")
        else:
            print("[rank-0]  Gate tensors to save:")
            for n, t in gate_state.items():
                print(f"  • {n:60s}  shape={tuple(t.shape)}")

        os.makedirs(out_dir, exist_ok=True)
        torch.save(gate_state, os.path.join(out_dir, "gate.pth"))
        print(f"[rank-0] Saved {len(gate_state)} tensors → {out_dir}/gate.pth")
