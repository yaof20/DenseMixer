    ...
    for rank, file in files:
        ...
        with open(os.path.join(args.expert_scores_dir, rank, file)) as f:
            ...
            for line in data:
                ...
                indicator = (expert_weights >= args.token_threshold).astype(int)
                np.add.at(token_scores[layer_id], expert_ids, indicator / TOP_K)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--token_threshold", type=float, required=True)
    ...
    summary = get_summary(file_names, args)
