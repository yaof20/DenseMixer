...
def parse_args():
    ...
    parser.add_argument("--exp_name", type=str, default="moe-finetune")
    parser.add_argument("--output_dir", type=str, required=False)
    parser.add_argument("--lora_router", type=str, default="frozen")
...
if __name__ == "__main__":
    ...
    if args.qlora:
        ...
        if args.lora_router != "frozen":
            print(f"Loading gate-only weights from {args.lora_model_name_or_path}/gate.pth")
            gate_state_dict = torch.load(f"{args.lora_model_name_or_path}/gate.pth", map_location="cpu")

            # Copy gate parameters one by one (only overwrite existing keys)
            with torch.no_grad():
                for name, gate_param in gate_state_dict.items():
                    try:
                        target_param = base_model.get_parameter(name)
                        if target_param.shape != gate_param.shape:
                            print(f"[WARN] shape mismatch {name}: {target_param.shape} vs {gate_param.shape}, skipped")
                            continue
                        target_param.copy_(gate_param.to(target_param.device))
                    except AttributeError:
                        print(f"[WARN] {name} not found in model, skipped")

            print("Successfully merged gate weights.")
    ...
    if args.tokenizer_name_or_path:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name_or_path, use_fast=args.use_fast_tokenizer, cache_dir=args.cache_dir)
    else:
        try:
            print("Trying to load the tokenizer in the lora model folder...")
            tokenizer = AutoTokenizer.from_pretrained(args.lora_model_name_or_path, use_fast=args.use_fast_tokenizer, cache_dir=args.cache_dir)
        except Exception as e:
            print(
                f"No tokenizer found in the lora model folder. Using the tokenizer in the base model folder... e:{e}"
            )
            tokenizer = AutoTokenizer.from_pretrained(args.base_model_name_or_path, use_fast=args.use_fast_tokenizer, cache_dir=args.cache_dir)
    ...
    lora_model = PeftModel.from_pretrained(base_model, args.lora_model_name_or_path, local_files_only=True)
