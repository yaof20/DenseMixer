...
def to_esft(model, adapter_config): # for olmoe
    ...
    elif hasattr(model, 'classifier'):  # if it's a classification task
        for param in model.classifier.parameters():
            param.requires_grad_(True)

    # Unfreeze selected experts
    for idx, layer in enumerate(model.model.layers):
        if type(layer.mlp).__name__ != "OlmoeSparseMoeBlock":
            continue
        trainable_experts = adapter_config['experts'][str(idx + 1)]
        for expert_id, expert in enumerate(layer.mlp.experts):
            expert_trainable = expert_id in trainable_experts
            for param in expert.parameters():
                param.requires_grad_(expert_trainable)
